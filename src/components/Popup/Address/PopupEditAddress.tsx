...
import { updateAddress, AddressData } from "@/lib/api/fetch-address";
import toast from "react-hot-toast";

interface PopupEditAddressProps {
      onClose: () => void;
      onSuccess?: () => void;
...
export function PopupEditAddress({ onClose, onSuccess, addressData }: PopupEditAddressProps) {
      const [isMobile, setIsMobile] = useState(false);
      const [loading, setLoading] = useState(false);
      
      // Form state - initialize with existing data
      const [label, setLabel] = useState(addressData.label);
      const [receiveName, setReceiveName] = useState(addressData.receiveName);
      const [phone, setPhone] = useState(addressData.phone);
      const [street, setStreet] = useState(addressData.street);
      const [kodePos, setKodePos] = useState(addressData.region?.postalCode || "");
      
      // Region state - initialize with existing data
      const [selectedProvinsi, setSelectedProvinsi] = useState<string | null>(addressData.region?.provinceCode || null);
      const [selectedKabupaten, setSelectedKabupaten] = useState<string | null>(addressData.region?.cityCode || null);
      const [selectedKecamatan, setSelectedKecamatan] = useState<string | null>(addressData.region?.districtCode || null);
      
      // Store region names - initialize with existing data
      const [regionData, setRegionData] = useState<RegionData>({
            provinceCode: addressData.region?.provinceCode || "",
            provinceName: addressData.region?.provinceName || "",
            cityCode: addressData.region?.cityCode || "",
            cityName: addressData.region?.cityName || "",
            districtCode: addressData.region?.districtCode || "",
            districtName: addressData.region?.districtName || "",
      });
      ...
      const handleSubmit = async () => {
            ...
            try {
                  const updatedData = {
                        ...
                        merchantId: addressData.merchantId || "string",
                  };

                  const response = await updateAddress(addressData.id, updatedData);
                  ...
                  if (response.success) {
                        ...
                        throw new Error(response.message || "Gagal memperbarui alamat");
                  }
            } catch (error) {
                  ...
                  >
                        <div className="flex flex-col gap-2 overflow-y-auto h-full">
                              <div className="inline-flex w-full justify-start items-center md:justify-between shadow-md md:shadow-none p-5 md:p-0 gap-3 md:gap-0 text-neutral-700">
                                    ...
                                    <h3 className="text-sm font-bold text-center">Edit Alamat</h3>
